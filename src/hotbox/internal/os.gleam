import gleam/string
import hotbox/result.{type HotboxError, OSError, ShelloutError, UnknownOSError, UnsupportedError, EUnreachable}
import shellout
import gleam/int
import gleam/float
...
pub fn get_available_memory(operating_system: String) -> Result(Int, HotboxError) {
...
fn darwin_get_available_memory() {
  let result = shellout.command(run: "memory_pressure", with: [], in: ".", opt: [])
  case result {
    Ok(output) -> {
      case string.split_once(string.drop_start(output, 15), " (") {
        Ok(#(ram_string, rest)) -> {
          case string.split_once(rest, "System-wide memory free percentage:") {
            Ok(#(_, chance_string)) -> case int.parse(ram_string), float.parse("0."<> string.drop_end(string.trim(chance_string), 1)) {
              Ok(ram), Ok(string) -> Ok(float.round(int.to_float(ram) *. string))
...
fn windows_get_available_memory() -> Result(Int, HotboxError) {
  let result = shellout.command(run: "powershell", with: ["-Command", "(Get-CimInstance Win32_OperatingSystem).FreePhysicalMemory"], in: ".", opt: [])
  case result {
    Ok(output) -> case int.parse(output) {
      Ok(memory) -> Ok(memory)
